...
export default class GameBoard {
  ...
  removeFromInventory(position) {
    ...
    this.tilesPlayedOnThisTurn = this.tilesPlayedOnThisTurn.filter(
      (t) => !t.position.equals(position)
    );
  }

  validateNewTilePosition(position) {
    if (this.inventory.length === 0) return true;
    return this.inventory.every((item) => !item.position.equals(position));
  }

  applyShaderAfterEndTurn(positions, valid) {
    positions.forEach((index) => {
      this.inventory[index].material = valid
        ? this.validShader.shader
        : this.invalidShader.shader;
    });
    setTimeout(() => {
      positions.forEach((index) => {
